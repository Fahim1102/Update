...
def fuck():
    ...
    with ThreadPool(max_workers=30) as asha:
        ...
        for love in user:
            ...
            asha.submit(FAHIM,uid,pwx,tl)
    print('-------------------')
    print(' [✓] Crack process has been completed')
    print(' [✓] OK Ids saved as FAHIM-OK.txt')
    print(' [✓] CP Id Save as FAHIM-CP.txt')
    print('-------------------')
def FAHIM(uid,pwx,tl):
    global loop
    global cps
    global oks
    global proxy
    try:
        for ps in pwx:
            ...
            sys.stdout.write('\r[FAHIM]--[%s/%s]--[OK-%s]~[CP-%s] \r'%(loop,tl,len(oks),len(cps))),
            sys.stdout.flush()
